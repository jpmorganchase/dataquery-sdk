...
def _event(
    ...
    *,
    ...
    if raw_data is not None:
        return SSEEvent(event=event_type, data=raw_data, id=event_id)
    body: Dict[str, Any] = {}
    if file_group_id is not None:
        body["file-group-id"] = file_group_id
    if file_date_time is not None:
        body["file-datetime"] = file_date_time
    if group_id is not None:
        body["group-id"] = group_id
    return SSEEvent(event=event_type, data=json.dumps(body), id=event_id)
...
@pytest.mark.asyncio
async def test_notification_with_file_updated_event_type(tmp_path):
    """Event type 'file-updated' triggers download."""
    client = _FakeClient()
...
@pytest.mark.asyncio
async def test_notification_skips_non_update_event_types(tmp_path):
    """Non-update event types like 'heartbeat' should not trigger downloads."""
    client = _FakeClient()
...
@pytest.mark.asyncio
async def test_download_error_goes_to_error_callback(tmp_path):
    client = _FakeClient()
    client.download_file_async.side_effect = RuntimeError("no network")
