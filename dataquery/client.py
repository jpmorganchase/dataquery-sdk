...
from .exceptions import (
    ...
    RateLimitError, NetworkError, ConfigurationError, FileNotFoundError
)
...
def get_filename_from_response(response: aiohttp.ClientResponse, file_group_id: str, file_datetime: Optional[str] = None) -> str:
    """
...
class DataQueryClient:
    ...
    def _build_api_url(self, endpoint: str) -> str:
        ...
        if len(url) > max_url_length:
            raise ValidationError(f"URL length ({len(url)}) exceeds maximum allowed ({max_url_length} characters). "
        ...
        if self.session is None:
            ...
            if self.config.proxy_enabled and self.config.has_proxy_credentials:
                try:
                    ...
                    _ = BasicAuth(login=self.config.proxy_username or "", password=self.config.proxy_password or "")
                except Exception:
                    pass
    ...
    def _validate_request_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> None:
        ...
        if len(complete_url) > max_url_length:
            raise ValidationError(f"Complete request URL length ({len(complete_url)}) exceeds maximum allowed ({max_url_length} characters). "
                                f"Consider reducing parameter values.",
                                details={"url_length": len(complete_url), "max_length": max_url_length, "url": complete_url[:200] + "..."})
    ...
    async def _execute_request(self, method: str, url: str, **kwargs) -> aiohttp.ClientResponse:
        ...
        if self.config.proxy_enabled and self.config.proxy_url:
            ...
            if self.config.has_proxy_credentials:
                from aiohttp import BasicAuth
                kwargs.setdefault('proxy_auth', BasicAuth(
    ...
    async def list_all_groups_async(self) -> List[Group]:
        ...
        all_groups = []
        next_url = self._build_api_url('groups')
        page_count = 0
        ...
        try:
            while next_url:
                ...
                async with await self._make_authenticated_request('GET', next_url) as response:
                    ...
                    self.logger.info("Groups page fetched", 
                                   page=page_count, 
            ...
            self.logger.info("All groups fetched", 
                           total_groups=len(all_groups), 
    ...
    async def search_groups_async(
        ...
        try:
            async with await self._make_authenticated_request('GET', url, params=params) as response:
                ...
                group_list = GroupList(**data)
                self.logger.info("Groups searched", 
                               keywords=keywords, 
                               count=len(group_list.groups))
    ...
    async def _download_file_single_stream(
        ...
        try:
            ...
            _req = self._make_authenticated_request('GET', url, params=params, headers=headers)
            try:
                cm = await _req
            except TypeError:
                cm = _req
            async with cm as response:
                ...
                return DownloadResult(
                    ...
                    local_path=destination if isinstance(destination, Path) else Path(self.config.download_dir) / f"{file_group_id}.tmp",
    ...
    async def list_available_files_async(
        ...
        try:
            async with await self._make_authenticated_request('GET', url, params=params) as response:
                ...
                self.logger.info("Available files listed", 
                               group_id=group_id, 
            ...
            self.logger.error("Failed to list available files", 
                            group_id=group_id, 
                            error=str(e))
            raise
    ...
    async def _enter_request_cm(self, method: str, url: str, **kwargs):
        ...
        try:
            cm = await req  # if coroutine returning CM
        except TypeError:
            cm = req       # already a CM
        return cm
    ...
    def _get_file_extension(self, file_group_id: str) -> str:
        ...
        suspicious_patterns = ['..', '/', '\\', '%2F', '%5C', 'etc/passwd', 'system32', 'config']
    ...
    def start_auto_download(
        ...
        """
        return asyncio.run(self.start_auto_download_async(
            group_id, destination_dir, interval_minutes, file_filter,
            progress_callback, error_callback, max_retries, check_current_date_only
        ))
    ...
    def to_dataframe(
        ...
        return self._convert_to_dataframe(
            response_data, flatten_nested, include_metadata,
            date_columns, numeric_columns, custom_transformations
        )
    ...
    def _auto_convert_columns(self, df: 'pd.DataFrame') -> 'pd.DataFrame':
        ...
        for column in df.columns:
            if df[column].dtype == 'object':  # String columns
                ...
                if any(date_word in column_lower for date_word in [
                    'date', 'time', 'created', 'updated', 'modified', 'expires'
                ]):
                    try:
                ...
                if any(num_word in column_lower for num_word in [
                    'size', 'count', 'bytes', 'price', 'value', 'amount', 
                    'volume', 'quantity', 'number', 'id'
                ]):
                    try:
    ...
    def time_series_to_dataframe(
        ...
        return self.to_dataframe(
            ...
            numeric_columns=['value', 'price', 'volume', 'open', 'high', 'low', 'close']
        )
    ...
    def download_file(
        ...
        """Synchronous wrapper using an event-loop aware runner."""
        return self._run_sync(self.download_file_async(file_group_id, file_datetime, options, progress_callback))
    ...
    def get_instrument_time_series(
        ...
        """Synchronous wrapper using an event-loop aware runner."""
        return self._run_sync(self.get_instrument_time_series_async(
    ...
    def get_expressions_time_series(
        ...
        """Synchronous wrapper using an event-loop aware runner."""
        return self._run_sync(self.get_expressions_time_series_async(
    ...
    def get_group_time_series(
        ...
        """Synchronous wrapper using an event-loop aware runner."""
        return self._run_sync(self.get_group_time_series_async(
