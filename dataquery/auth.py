...
class TokenManager:
    ...
    def _setup_token_storage(self):
        ...
        # Prefer explicit token_storage_dir, else fallback to download_dir/.tokens
        base_dir: Optional[Path] = None
        if getattr(self.config, 'token_storage_dir', None):
            base_dir = Path(self.config.token_storage_dir)
...
class OAuthManager:
    ...
    def is_authenticated(self) -> bool:
        ...
        return (
            self.config.has_bearer_token or 
            self.config.has_oauth_credentials
        )
