...
from .models import (
    ...
    AvailabilityInfo, DownloadOptions, InstrumentsResponse, 
...
from .client import format_file_size, format_duration
...
class DataQuery:
    ...
    async def run_group_download_parallel_async(
        ...
        logger.info("=== Starting Group Parallel Download for Date Range Operation ===",
                   group_id=group_id, start_date=start_date, end_date=end_date, 
                   max_concurrent=max_concurrent, num_parts=num_parts,
        ...
        try:
            ...
            if not filtered_files:
                ...
                return {
                    ...
                    "total_time_formatted": f"{int(total_time_minutes)}m {int(total_time_seconds % 60)}s" if total_time_minutes >= 1 else f"{total_time_seconds:.1f}s"
                }
            ...
            logger.info("Using delay-based rate limit protection with full concurrency", 
            ...
            async def download_file_with_flattened_concurrency(file_info, delay_seconds: float = 0.0):
                file_group_id = file_info.get('file-group-id', file_info.get('file_group_id'))
                file_datetime = file_info.get('file-datetime', file_info.get('file_datetime'))
                ...
                if delay_seconds > 0:
                    logger.debug("Applying delay before download", 
                               file_group_id=file_group_id, delay_seconds=delay_seconds)
                ...
                try:
                    ...
                    logger.info("Downloaded file (flattened concurrency)", 
                               file_group_id=file_group_id,
                               file_datetime=file_datetime, 
                               status=result.status.value if result else "failed")
                    return result
                except Exception as e:
                    logger.error("Flattened parallel download failed", 
                               file_group_id=file_group_id,
                               file_datetime=file_datetime, error=str(e))
                    return None
            ...
            for i, file_info in enumerate(filtered_files):
                ...
                task = download_file_with_flattened_concurrency(file_info, delay_seconds)
                download_tasks.append(task)
            
            logger.info("Starting downloads with intelligent delay-based rate limiting", 
                       total_files=len(filtered_files),
                       base_delay=delay_between_downloads,
                       intelligent_delay=intelligent_delay,
            ...
            for file_info, result in zip(filtered_files, download_results):
                ...
                elif result and hasattr(result, 'status') and result.status.value == "completed":
            ...
            report = {
                ...
                "per_file_timing": {
                    ...
                    "total_download_time_formatted": f"{int(total_download_time // 60)}m {int(total_download_time % 60)}s" if total_download_time >= 60 else f"{total_download_time:.1f}s"
                }
            }
            logger.info("Group parallel download for date range operation completed!", **report)
            return report
        except Exception as e:
            logger.error("Group parallel download for date range operation failed",
                       group_id=group_id, start_date=start_date, end_date=end_date, error=str(e))
            raise
    ...
    async def _download_file_parallel_flattened(
        ...
        """
        from .models import DownloadOptions, DownloadResult, DownloadStatus, DownloadProgress
        ...
        try:
            ...
            async with global_semaphore:  # Use global semaphore for probe request
                async with (await self._client._enter_request_cm('GET', url, params=params, headers=probe_headers)) as probe_resp:
            ...
            async def download_range_with_global_semaphore(start_byte: int, end_byte: int):
                nonlocal bytes_downloaded
                headers = {"Range": f"bytes={start_byte}-{end_byte}"}
                
                # Each range request uses the global semaphore
                async with global_semaphore:
                    async with (await self._client._enter_request_cm('GET', url, params=params, headers=headers)) as resp:
            ...
            await asyncio.gather(*(download_range_with_global_semaphore(s, e) for s, e in ranges))
            ...
            logger.error("Flattened parallel download failed for file", 
                        file_group_id=file_group_id, error=str(e))
            return None
    ...
    def _calculate_rate_limit_capacity(self) -> Dict[str, Any]:
        ...
        """
        if not self._client:
            return {"requests_per_minute": 1000, "burst_capacity": 100, "safe_interval": 0.1}
        ...
        try:
            ...
            logger.warning("Failed to calculate rate limit capacity, using conservative defaults", error=str(e))
            return {"requests_per_minute": 100, "burst_capacity": 10, "safe_interval": 0.6}
    ...
    def _calculate_intelligent_delay(
        ...
        try:
            ...
            if total_concurrent_requests > burst_capacity:
                ...
                logger.debug("Burst capacity exceeded, calculating spread delay",
                           total_concurrent_requests=total_concurrent_requests,
                           burst_capacity=burst_capacity,
            ...
            logger.debug("Intelligent delay calculation details",
            ...
            logger.info("Calculated intelligent delay for rate limit protection",
                       total_concurrent_requests=total_concurrent_requests,
                       requests_per_minute=requests_per_minute,
                       burst_capacity=burst_capacity,
                       safe_interval=safe_interval,
    ...
    def _calculate_safe_concurrency_limit(self, requested_concurrency: int) -> int:
        ...
        try:
            ...
            if not rate_config.enable_rate_limiting:
                logger.info("Rate limiting disabled, using requested concurrency",
            ...
            if safe_limit < requested_concurrency * 0.5:
                logger.warning("Significantly reducing concurrency due to rate limits",
                             requested=requested_concurrency,
            ...
            logger.info("Calculated safe concurrency limit",
                       requested=requested_concurrency,
                       safe_limit=safe_limit,
                       rate_limit_rpm=requests_per_minute,
                       burst_capacity=burst_capacity,
            ...
            logger.warning("Failed to calculate safe concurrency limit, using conservative default",
                         error=str(e), requested=requested_concurrency)
    ...
    def download_file_sync(
        ...
        return asyncio.run(self.download_file_async(file_group_id, file_datetime, destination_path, options, progress_callback))
